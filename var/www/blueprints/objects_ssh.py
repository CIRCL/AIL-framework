...
# ============ BLUEPRINT ============
objects_ssh = Blueprint('objects_ssh', __name__, template_folder=os.path.join(os.environ['AIL_FLASK'], 'templates/objects/ssh'))
...
# ============= ROUTES ==============
@objects_ssh.route("/objects/ssh_keys", methods=['GET'])
@login_required
@login_read_only
def objects_ssh_keys():
    ...
    if show_objects:
        dict_objects = SSHKeys.SSHKeys().api_get_meta_by_daterange(date_from, date_to)
    else:
        dict_objects = {}

    return render_template("SSHKeyDaterange.html", date_from=date_from, date_to=date_to,
                           dict_objects=dict_objects, show_objects=show_objects)

@objects_ssh.route("/objects/ssh_key/post", methods=['POST'])
@login_required
@login_read_only
def objects_ssh_keys_post():
    date_from = request.form.get('date_from')
    date_to = request.form.get('date_to')
    show_objects = request.form.get('show_objects')
    return redirect(url_for('objects_ssh.objects_ssh_keys', date_from=date_from, date_to=date_to, show_objects=show_objects))

@objects_ssh.route("/objects/ssh_key/range/json", methods=['GET'])
@login_required
@login_read_only
def objects_ssh_key_range_json():
    date_from = request.args.get('date_from')
    date_to = request.args.get('date_to')
    date = Date.sanitise_date_range(date_from, date_to)
    date_from = date['date_from']
    date_to = date['date_to']
    return jsonify(SSHKeys.SSHKeys().api_get_chart_nb_by_daterange(date_from, date_to))

# @objects_ssh.route("/objects/ssh_key/search_post", methods=['POST'])
# @login_required
# @login_user
...
# @objects_ssh.route("/objects/ssh_key/search", methods=['GET'])
# @login_required
# @login_user
